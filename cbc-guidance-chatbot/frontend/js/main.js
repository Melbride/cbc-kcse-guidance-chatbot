...
const API_BASE = (() => {
    ...
    // Development environments - explicit check for frontend on port 3000
    if ((hostname === 'localhost' || hostname === '127.0.0.1') && (port === '3000' || !port)) {
        console.log('Using development API: http://localhost:8000');
    ...
    if (hostname.includes('netlify.app') || hostname.includes('netlify.com')) {
        ...
        console.log('Using production API: https://cbc-kcse-guidance-chatbot.onrender.com');
...
function showGuestLimitModal() {
  ...
  `;  // FIX 1: Was missing closing backtick+semicolon for innerHTML, had `}` instead

  document.body.appendChild(modal);
} // FIX 2: Was missing closing `}` for the function entirely
...
} // FIX 3: Was closed with `)` instead of `}`
...
function getPostLoginRedirect() {
  try {
    ...
    if (next && !next.includes('://') && !next.startsWith('//')) {
      return next;
    } // FIX 4: Was missing closing `}` for the if-block
  } catch {
    // Ignore
  }
  return ''; // FIX 5: The return '' was orphaned outside the try; moved inside properly
}
...
async function testAPIConnection() {
  ...
  try {
    const response = await fetch(`${API_BASE}/users/email/test@example.com`, {
      method: 'GET',
      headers: {
...
async function login() {
  ...
  try {
    ...
    const checkUrl = `${API_BASE}/users/email/${encodeURIComponent(email)}`;
    ...
    if (checkResponse.ok) {
      ...
      console.error('Server error:', checkResponse.status, errorText);
      alert(`Error checking user (${checkResponse.status}): ${errorText}`);
    }
  } catch (error) {
    console.error('Login error:', error);
    alert(`Network error: ${error.message}. Please check:\n1. Backend server is running on port 8000\n2. No firewall blocking localhost\n3. Browser console for more details`);
  }
}
